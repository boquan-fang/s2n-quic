...
mod tests {
    ...
    impl s2n_quic_core::event::Subscriber for TestStatsSubscriber {
        ...
        fn on_platform_rx_socket_stats(
            ...
            // On the first socket event (any socket — real client hello received),
    ...
    ///
    /// The flood thread is parked until the server receives the real client hello,
    /// then unparked to start flooding. A 15-second timeout prevents the test from
    /// hanging if the handshake can't complete.
    #[cfg(target_os = "linux")]
    #[tokio::test]
    async fn dc_server_packet_filtering_load_test() {
        ...
        // Wrap the handshake in a 15-second timeout so the test doesn't hang forever
        // if the handshake can't complete under flood.
        let handshake_result = tokio::time::timeout(Duration::from_secs(15), async {
            client
        ...
        // Unpark the flood thread in case FloodSync never triggered (e.g., when
        // PlatformRxSocketStats is not emitted because the two-socket setup is absent).
        // Without this, the thread stays parked and join() blocks forever.
        flood_thread.thread().unpark();
        ...
        // With the router attached, the handshake should succeed.
        // Without the router, this will timeout — proving the router is necessary.
        match &handshake_result {
            Ok(Ok(())) => tracing::info!("Handshake succeeded under flood"),
            Ok(Err(e)) => tracing::error!("Handshake failed: {e:?}"),
            Err(_) => tracing::error!("Handshake timed out after 15 seconds"),
        }

        let inner = handshake_result
            .expect("Handshake timed out — the router is likely not filtering packets");
        assert!(inner.is_ok(), "Handshake failed under flood");
    }
}
