...
impl<Config: endpoint::Config> Manager<Config> {
    ...
    /// last state the handshake reached (for example, a server left stuck in
    /// `ServerTokensSent` because it never received the client's ACK).
    ///
    ...
    /// completed and the connection closed cleanly, yet the dc handshake never
    /// finished and nothing else signals that anything went wrong.
    pub fn on_close<Pub: event::ConnectionPublisher>(
        ...
        // A disabled `dc::Manager` is initialized to `Complete`, so this also
        // skips connections where dc was not negotiated.
