...
fn deduplicate_successfully() {
    ...
    test(model, |handle| {
        let mut server = Server::builder()
            ...
            .with_event((tracing_events(None), server_subscriber.clone()))?
        ...
        let mut server2 = Server::builder()
            .with_io(handle.builder().build()?)?
            .with_tls(SERVER_CERTS)?
            .with_event((tracing_events(None), server_subscriber))?
            .with_random(Random::with_seed(456))?
            .start()?;
        ...
        let client = Client::builder()
            ...
            .with_event((tracing_events(None), client_subscriber))?
            .with_random(Random::with_seed(456))?
...
fn confirm_conn_works(
    conn: &mut s2n_quic::connection::Connection,
